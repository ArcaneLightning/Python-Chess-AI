...
class Heuristics:
    ...
    # The piece square tables denote the points scored for the position of the chess pieces on the board.
    # These were accessed from the internet due to the stastical and mathematical knowledge needed to create them
...
class AI:
    ...
    @staticmethod
    def get_ai_move(chessboard, difficulty, color):
        ...
        for move in chessboard.remove_check_moves(color):
            copy = board.Board.clone(chessboard)
            copy.perform_move(move)

            if color == pieces.Piece.BLACK:
                score = AI.alphabeta(copy, difficulty, -AI.INFINITE, AI.INFINITE, True)
                if (score < best_score):
                    best_score = score
                    best_move = move

            elif color == pieces.Piece.WHITE:
                score = AI.alphabeta(copy, difficulty, -AI.INFINITE, AI.INFINITE, False)
                if (score > best_score):
                    best_score = score
                    best_move = move

        return best_move #Best move is 0 if checkmate
    ...
    def minimax(board, depth, maximizing):
        ...
        if (maximizing):
            best_score = -AI.INFINITE
            for move in board.remove_check_moves(pieces.Piece.WHITE):
            ...
            best_score = AI.INFINITE
            for move in board.remove_check_moves(pieces.Piece.BLACK):
                copy = board.Board.clone(board)
                copy.perform_move(move)
    ...
    def alphabeta(chessboard, depth, a, b, maximizing):
        ...
        if (maximizing):
            best_score = -AI.INFINITE
            for move in chessboard.remove_check_moves(pieces.Piece.WHITE):
                copy = board.Board.clone(chessboard)
                copy.perform_move(move)
            ...
            best_score = AI.INFINITE
            for move in chessboard.remove_check_moves(pieces.Piece.BLACK):
                copy = board.Board.clone(chessboard)
                copy.perform_move(move)
